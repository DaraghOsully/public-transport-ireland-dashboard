...
function yoyFor(weekly, year, week, mode = null) {
  ...
  const cur = pick(year, week), prev = pick(year - 1, week);
  if (!cur || !prev) return null;
  const a = mode ? cur.modes?.[mode] : cur.value, b = mode ? prev.modes?.[mode] : prev.value;
  return Number.isFinite(a) && Number.isFinite(b) && b !== 0 ? (a - b) / b : null;
}

export default function App() {
  const [data, setData] = useState(null), [error, setError] = useState('');
  const [selectedMode, setSelectedMode] = useState('All Public Transport'), [year, setYear] = useState('all'), [range, setRange] = useState(24);

  useEffect(() => {(async () => { try {
    const [tha, tii] = await Promise.all([csv('THA25.csv'), csv('TII03.csv')]);
    const thaModes = transformTHA25(tha), maxWeekByYear = {};
    for (const r of thaModes) maxWeekByYear[r.year] = Math.max(maxWeekByYear[r.year] ?? 0, r.week);
    setData(validateNonLuas(aggregateByWeek([...thaModes, ...transformTII03(tii, maxWeekByYear)]), officialNonLuasByWeek(tha)));
  ...
  const latest = filtered.at(-1), modeValue = (r, mode) => mode === 'All Public Transport' ? r.value : (r.modes?.[mode] ?? 0);
  ...
  return <main className="shell">
    ...
    <section className="controls"><label>Measure<select value={selectedMode} onChange={e=>setSelectedMode(e.target.value)}><option>All Public Transport</option>{Object.values(MODES).map(m=><option key={m}>{m}</option>)}</select></label><label>Year<select value={year} onChange={e=>setYear(e.target.value)}><option value="all">All years</option>{years.map(y=><option key={y}>{y}</option>)}</select></label><label>Weeks shown<input type="range" min="8" max="104" value={range} onChange={e=>setRange(Number(e.target.value))}/><span>{range} weeks</span></label></section>
    <section className="cards"><article><span>Latest week</span><strong>{fmt.format(modeValue(latest, selectedMode))}</strong><small>passenger journeys</small></article><article><span>Year on year</span><strong className={trend > 0 ? 'up' : trend < 0 ? 'down' : ''}>{trend == null ? '—' : `${trend > 0 ? '+' : ''}${pct.format(trend)}`}</strong><small>same ISO week last year</small></article><article><span>Data validation</span><strong className={validation?.matches ? 'up' : 'down'}>{validation?.matches ? '✓ Match' : validation?.official == null ? '—' : 'Check'}</strong><small>THA25 official non-Luas total</small></article></section>
    <section className="insight-grid"><div className="insight-main"><span className="eyebrow">WHAT CHANGED?</span><h2>{insights[0]?.text ?? 'No comparable insight available yet.'}</h2><p>Insights are calculated directly from the comparable weekly observations — not written by hand.</p></div><div className="insight-side"><span className="eyebrow">LATEST PERIOD</span><strong>{latest.year} · Week {String(latest.week).padStart(2,'0')}</strong><span>{filtered.length} observations in view</span></div></section>
    <section className="panel"><div className="panel-head"><div><span className="eyebrow">TREND</span><h2>Weekly passenger journeys</h2></div><span>{filtered.length} observations</span></div><div className="chart" role="img" aria-label="Weekly passenger journeys bar chart">{filtered.map(r => <div className="bar-wrap" key={`${r.year}-${r.week}`} title={`${r.year} W${r.week}: ${fmt.format(modeValue(r, selectedMode))}`}><div className="bar" style={{height:`${Math.max(2, modeValue(r,selectedMode)/Math.max(...filtered.map(x=>modeValue(x,selectedMode)))*100)}%`}}/><small>{r.week === 1 ? r.year : ''}</small></div>)}</div></section>
